...
fn main() {
    ...
    // let matches = Command::new("Cracker")
    //     .about("Tries to reveal the original content of a hashed value.")
    //     .arg(Arg::new("length")
    //         .required(false)
    //         .takes_value(true)
    ...
    //         .long("all")
    //         .short('a')
    //         .help("List all possible algorithms."))
    //     .get_matches();
    ...
    // print::help();
    //TODO: Fix password_info so its a global variable.
    hash::get_possible_algorithm(input);
...
mod hash {
    use crate::prelude::*;
    pub fn get_possible_algorithm(input: &str) -> Result<(hasher::HashType, hasher::HashType), String> {
        let hash;
